...
class Job(Base):
    ...
    intrest_user = db.relationship(
        'User', secondary=user_jobs, backref=db.backref('interested'), lazy=True)
    ...
    def interested_jobs(accountID):
        ...
        response = []
        for row in res:
            response.append({"id": row[0], "name": row[1], "salary": row[2],
                             "description": row[3], "interested": row[4], "account_id": row[5]})
